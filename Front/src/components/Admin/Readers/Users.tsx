...
import actGetReaders from '@store/usersSlice/act/actGetReaders';
...
function Comment() {
    ...
    const {loading,error,readers} = useAppSelector(state => state.users);
  
    const dispatch = useAppDispatch();
    const [users, setUsersList] = useState<TUser[] | TUserAra[]>([]);
    useEffect(() => {
        dispatch(actGetReaders())
    }, [language])
    useEffect(() => {
        if (readers) {
        const getAllUsers = () => {
            const user: TUser[] = readers.map((user) => {
                    return ({
                        id: user.id!,
                        name: user.name!,
            ...
            const userAra: TUserAra[] = readers.map((user) => {
                    return ({
                        id: user.id!,
                        الاسم: user.name,
