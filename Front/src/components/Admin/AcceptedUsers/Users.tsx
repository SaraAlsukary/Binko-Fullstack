...
import actGetAcceptedUsers from '@store/usersSlice/act/actGetAcceptedUsers';
...
function Comment() {
    ...
    const { loading, error, acceptedUsers } = useAppSelector(state => state.users);

    const dispatch = useAppDispatch();
    const [users, setUsersList] = useState<TUser[] | TUserAra[]>([]);
    useEffect(() => {
        dispatch(actGetAcceptedUsers())
    }, [language])
    useEffect(() => {
        if (acceptedUsers) {
            const getAllUsers = () => {
                const user: TUser[] = acceptedUsers.map((user) => {
                    return ({
                        id: user.id!,
                        name: user.name,
                ...
                const userAr: TUserAra[] = acceptedUsers.map((user) => {
                    return ({
                        id: user.id!,
                        الاسم: user.name,
                ...
                const data = language === 'Arabic' ? userAr : user
                setUsersList(data);
