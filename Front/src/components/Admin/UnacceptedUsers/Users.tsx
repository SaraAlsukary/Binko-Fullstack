...
import actGetUnacceptedUsers from '@store/usersSlice/act/actGetUnacceptedUsers';
...
function Comment() {
    ...
    const { loading, error, unacceptedUsers } = useAppSelector(state => state.users);

    const dispatch = useAppDispatch();
    const [users, setUsersList] = useState<TUser[] | TUserAra[]>([]);
    useEffect(() => {
        dispatch(actGetUnacceptedUsers())
    }, [language])
    useEffect(() => {
        if (unacceptedUsers) {
            const getAllUsers = () => {
                const user: TUser[] = unacceptedUsers.map((user) => {
                    return ({
                        id: user.id!,
                        name: user.name,
                ...
                const userAra: TUserAra[] = unacceptedUsers.map((user) => {
                    return ({
                        id: user.id!,
                        الاسم: user.name,
